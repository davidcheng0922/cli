...
import (
	...
	upgrade "github.com/longhorn/cli/pkg/upgrade"
)
...
func newCmdCheckUpgrade(globalOpts *types.GlobalCmdOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Check Longhorn upgrade readiness",
		Long: `This command performs a point-in-time check of Longhorn upgrade readiness for a specific data engine.
Run it immediately before the upgrade operation and avoid concurrent volume changes while checking.`,
	...
	cmd.Flags().StringVar(&checker.DataEngine, consts.CmdOptDataEngine, "", "Data engine to check (v1, v2)")
	...
	if err := cmd.MarkFlagRequired(consts.CmdOptDataEngine); err != nil {
		logrus.WithError(err).Warnf("Failed to mark option %s as required", consts.CmdOptDataEngine)
	}
